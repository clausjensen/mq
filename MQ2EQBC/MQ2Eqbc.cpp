// MacQ - April 14, 2015 - Amazing program originally written by Omnictrl back in 2005.
// Many contributions/support, keeping this program alive, from folks like PMS, ascii38, sOrcier, toomanynamnes, three-p-o, and dewey2461, just to name a few.
//
// MacQ - April 14, 2015 - With my limited c++ skills, compared to the folks above who know what they are doing,
// I added code to LoadINI() allowing variables to be set in [Settings] section or [SERVER.CHARACTER] section of the INI.
//
// Jimbob - July 30, 2015 - Added support for my eqbci application
//
...
const unsigned int MAX_READBUF        = MAX_STRING;
...
// Initialize from the [Settings] section of the INI.
...
// Initialize from [SERVER.CHARACTER] section of the INI.  If there is no corresponding variable in [SERVER.CHARACTER] section,
// or if there is no [SERVER.CHARACTER] section at all, the values initialized from the above [Settings] section, or the defaults
// in that section, will be used.
        AllowControl  = GetPrivateProfileInt(szCharName, "AllowControl",			AllowControl,	INIFileName);
        AutoConnect   = GetPrivateProfileInt(szCharName, "AutoConnect",				AutoConnect,	INIFileName);
        AutoReconnect = GetPrivateProfileInt(szCharName, "AutoReconnect",			AutoReconnect,	INIFileName);
        IRCMode       = GetPrivateProfileInt(szCharName, "IRCCompatMode",			IRCMode,		INIFileName);
        LocalEcho     = GetPrivateProfileInt(szCharName, "LocalEcho",				LocalEcho,		INIFileName);
        EchoAll       = GetPrivateProfileInt(szCharName, "EchoAll",					EchoAll,		INIFileName);
        NotifyControl = GetPrivateProfileInt(szCharName, "NotifyControl",			NotifyControl,	INIFileName);
        ReconnectSecs = GetPrivateProfileInt(szCharName, "ReconnectRetrySeconds",	ReconnectSecs,	INIFileName);
        SaveByChar    = GetPrivateProfileInt(szCharName, "SaveByCharacter",			SaveByChar,		INIFileName);
        SilentCmd     = GetPrivateProfileInt(szCharName, "SilentCmd",				SilentCmd,		INIFileName);
        SilentIncCmd  = GetPrivateProfileInt(szCharName, "SilentIncCmd",			SilentIncCmd,	INIFileName);
        SilentOutMsg  = GetPrivateProfileInt(szCharName, "SilentOutMsg",			SilentOutMsg,	INIFileName);
        WatchFsay     = GetPrivateProfileInt(szCharName, "FSWatch",					WatchFsay,		INIFileName);
        WatchGroup    = GetPrivateProfileInt(szCharName, "GroupWatch",				WatchGroup,		INIFileName);
        WatchGuild    = GetPrivateProfileInt(szCharName, "GuildWatch",				WatchGuild,		INIFileName);
        WatchTell     = GetPrivateProfileInt(szCharName, "TellWatch",				WatchTell,		INIFileName);
        Window        = GetPrivateProfileInt(szCharName, "UseWindow",				Window,			INIFileName);
...
public:
    ...
    CEQBCWnd(CXStr* Template) : CCustomWnd(Template)
    {
        ...
        CloseOnESC             = 0;
        InputBox->SetMaxChars(MAX_STRING);
...
public:
	...
	void BCGA(char* szLine, bool all=true)
    {
		int x=0;
        if (!ConnectReady()) return;
		if (!(GetCharInfo()->pGroupInfo))
		{
			WriteOut("You are not in a group.");
			return;
		}
		bool all_mercs=true;
		for (x=0; x<6; x++)
		{
			if(!GetCharInfo()->pGroupInfo->pMember) break;
			if(!GetCharInfo()->pGroupInfo->pMember[x]) break;
			if(!GetCharInfo()->pGroupInfo->pMember[0]->Mercenary) { all_mercs=false; break; }
		}
		if(all_mercs)
		{
			WriteOut("There are only mercenaries in your group.");
			return;
		}
        char szCmdBct[] = CMD_TELL;
		char szBcg[MAX_STRING];
		for (x=0; x<6; x++)
		{
			if(!GetCharInfo()->pGroupInfo->pMember) break;
			if(!GetCharInfo()->pGroupInfo->pMember[x]) break;
			if(GetCharInfo()->pGroupInfo->pMember[x]->Mercenary) continue;
	        if (szLine && strlen(szLine))
			{
				if((strcmp(GetCharInfo()->pGroupInfo->pMember[x]->pName->Text,GetCharInfo()->Name)!=0)||all)
					_snprintf(szBcg, MAX_STRING, "%s %s", GetCharInfo()->pGroupInfo->pMember[x]->pName->Text, szLine);
				ChanTransmit(szCmdBct, szBcg);
			}
		}
/*		if(all)
		{
			_snprintf(szBcg, MAX_STRING, "%s %s", GetCharInfo()->Name, szLine);
			ChanTransmit(szCmdBct, szBcg);
		}*/
        if (szLine && strlen(szLine))
        {
    ...
    void HandleBciMsg(char* szName, char* szMsg)
    {
        ...
        if (!strncmp(szMsg, "REQ", 3))
        {
            if (GetGameState() == GAMESTATE_INGAME && GetCharInfo()->pSpawn)
            {
                ...
                sprintf(szBuff, "1|%d|%d|%d|%d|%d|%d|%d|%d|%s|%s|%d|%d|%d|%c|%c|%s|%f|%f|%f|%s|%d|%d|%d|%d|%s", GetCurHPS(), GetMaxHPS(), GetCharInfo2()->Mana, GetMaxMana(), GetCharInfo2()->Endurance, GetMaxEndurance(),
                    GetCharInfo()->zoneId, GetCharInfo2()->Level, GetClassDesc(GetCharInfo2()->Class), pEverQuest->GetRaceDesc(GetCharInfo2()->Race), GetCharInfo()->Exp,
					GetCharInfo()->AAExp, GetCharInfo2()->AAPoints, ((PSPAWNINFO)pCharSpawn)->StandState, (((PSPAWNINFO)pCharSpawn)->PetID == 0xFFFFFFFF) ? '0' : '1', 
					((PZONEINFO)pZoneInfo)->ShortName, ((PSPAWNINFO)pCharSpawn)->X, ((PSPAWNINFO)pCharSpawn)->Y, ((PSPAWNINFO)pCharSpawn)->Z,
					...
					(GetCharInfo()->pGroupInfo) ? 1 : 0, (GetCharInfo()->pGroupInfo) ? GetCharInfo()->pGroupInfo->pLeader->pName->Text : "(null)");
